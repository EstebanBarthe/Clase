<?php
    ...
    $arrayColoresDos = array("Amarillo"=>"f3f47f","Rojo"=>"#d3221a","Verde"=>"18e22d");
...
//------------------------------------------
    ...
    $arrayDesordeonado = array(5,7,2,1,9,3);
    $arrayOrdenado = array();
    $numeroMenor = 0;
    $posicionNumero = 0;

    print_r(count($arrayDesordeonado));
    $totalArray = count($arrayDesordeonado);
    
    for($k=0; $k<$totalArray; $k++){
        for($i=0; $i<$totalArray; $i++){
            print_r("<br>".$arrayDesordeonado[$i]);
            if($arrayDesordeonado[$i] !=null && 
                    $arrayDesordeonado[$i] < $numeroMenor || $numeroMenor == 0){
                $numeroMenor = $arrayDesordeonado[$i];
                $posicionNumero = $i;
            }
        }
        $arrayOrdenado[] = $numeroMenor;
        $arrayDesordeonado[$posicionNumero] = null;
